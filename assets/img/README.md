...
No real art exists yet, so every image below is a solid-color placeholder
(see `assets/css/style.css`, the `.ap-media` component) until a real file
is dropped in at the matching path. Drop a file in at the exact path and
it's picked up automatically — nothing else needs to change.
...
| `story/landscape.jpg` | Long full-width landscape image, story-introduction section |
| `contact/author.jpg` | Author photo, contact/TOS section |
